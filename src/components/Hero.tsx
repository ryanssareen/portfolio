...
export function Hero() {
  ...
  return (
    <section
      id="hero"
      className="relative min-h-[100svh] md:min-h-screen flex items-center justify-center overflow-hidden py-28 md:py-0"
    >
      {/* Dot-grid background */}
      <div className="absolute inset-0 dot-grid -z-20" />

      {/* Radial fade so dots fade toward edges & center */}
      ...
      <div className="max-w-4xl mx-auto px-5 sm:px-8 text-center">
        ...
        <div className={`transition-all duration-700 ${mounted ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"}`}>
          <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full border border-green-200 dark:border-green-800/60 bg-green-50 dark:bg-green-900/20 text-green-700 dark:text-green-400 text-xs font-semibold mb-8 tracking-wide uppercase">
        ...
        <div className={`transition-all duration-700 delay-75 ${mounted ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"}`}>
          <p className="text-sm sm:text-base text-foreground/40 font-medium mb-4 tracking-widest uppercase">
            Ryan Singh Sareen
          </p>
        </div>
        ...
        <div className={`transition-all duration-700 delay-150 ${mounted ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"}`}>
          <h1 className="text-5xl sm:text-6xl md:text-7xl lg:text-8xl font-bold leading-[1.02] mb-6">
        ...
        <div className={`transition-all duration-700 delay-200 ${mounted ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"}`}>
          <p className="text-base sm:text-lg md:text-xl text-foreground/50 mb-10 max-w-lg mx-auto leading-relaxed font-light">
        ...
        <div className={`transition-all duration-700 delay-300 ${mounted ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"}`}>
          <div className="flex flex-col sm:flex-row items-center justify-center gap-3 mb-12">
            ...
            <Link
              href="/contact"
              className="w-full sm:w-auto px-8 py-3.5 rounded-xl font-semibold text-sm border border-slate-200 dark:border-slate-700 text-foreground/70 hover:text-foreground hover:border-slate-400 dark:hover:border-slate-500 hover:-translate-y-0.5 bg-white/50 dark:bg-white/[0.02] transition-all duration-200"
            >
              Get in Touch
            </Link>
          </div>
        ...
        <div className={`transition-all duration-700 delay-[400ms] ${mounted ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"}`}>
          <div className="inline-flex items-center rounded-2xl border border-slate-200/70 dark:border-slate-700/50 bg-white/60 dark:bg-white/[0.03] backdrop-blur-sm divide-x divide-slate-200/70 dark:divide-slate-700/50 overflow-hidden shadow-sm">
            {stats.map((s) => (
              <div key={s.label} className="flex flex-col items-center px-6 py-3.5">
        ...
        <div className={`transition-all duration-700 delay-500 ${mounted ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"}`}>
          <div className="flex items-center justify-center gap-4 mt-8">
            <a
              ...
              className="flex items-center gap-1.5 text-foreground/35 hover:text-foreground/75 text-xs font-medium transition-colors"
            >
            ...
            <span className="w-px h-3 bg-slate-300 dark:bg-slate-700" />
            <span className="flex items-center gap-1.5 text-foreground/35 text-xs font-medium">
